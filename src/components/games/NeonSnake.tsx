...
const SNAKE_COLORS = [
  '#ff00ff', '#00ffff', '#ffff00', '#ff0000', '#00ff00', '#0000ff', '#ff8800', '#8800ff'
];

export function NeonSnake({ channels, isHost, myId, myName, guests, onBackToLobby }: NeonSnakeProps) {
  ...
  const networkInputs = useRef<Record<string, { targetAngle: number, boost: boolean }>>({});
  ...
  useEffect(() => {
    ...
    const mkSnake = (id: string, name: string, isBot: boolean, idx: number): SnakeState => {
  ...
  const broadcastSync = (state: GameState) => {
      channels.forEach(channel => {
        if (channel.readyState === 'open') {
          channel.send(JSON.stringify({ type: 'NEON_SNAKE_SYNC', payload: state }));
        }
      });
  };
  ...
  const startMatch = () => {
    ...
    guests.forEach(g => {
        ...
        networkInputs.current[g.id] = { targetAngle: snakes[g.id].dir, boost: false };
    });
  ...
  const hostTick = () => {
    ...
    const activeSnakes: SnakeState[] = Object.values(state.snakes).filter((s: any) => !s.isDead);

    // Bot Logic
    activeSnakes.filter((s: SnakeState) => s.isBot).forEach((bot: SnakeState) => {
        ...
        if (Math.random() < 0.05) {
            ...
            for (const f of state.foods) {
                const dist = (f.x - bot.segments[0].x)**2 + (f.y - bot.segments[0].y)**2;
                if (dist < closeDist) {
                    closeDist = dist;
                    closeFood = f;
                }
            ...
            if (closeFood && closeDist < 300000) {
               bot.targetAngle = Math.atan2(closeFood.y - bot.segments[0].y, closeFood.x - bot.segments[0].x);
            } else {
        ...
        if (head.x < wallMargin || head.x > WORLD_SIZE - wallMargin || head.y < wallMargin || head.y > WORLD_SIZE - wallMargin) {
    ...
    // Check Win Condition
    const aliveHumans = (Object.values(state.snakes) as SnakeState[]).filter(s => !s.isDead && !s.isBot);
    const aliveTotal = (Object.values(state.snakes) as SnakeState[]).filter(s => !s.isDead);

    if (aliveTotal.length <= 1) { // 1 or 0 left
    ...
    const sendInput = () => {
        if (isHost) {
            ...
            if (ch && ch.readyState === 'open') {
               ch.send(JSON.stringify({ type: 'NEON_SNAKE_INPUT', payload: { targetAngle: localInput.current.targetAngle, boost: localInput.current.boost } }));
            }
        }
    };
  ...
  useEffect(() => {
    ...
    ctx.save();
    // Center camera on my snake
    let cx = camera.x;
    let cy = camera.y;

    // Transform setup
    ctx.translate(viewport.w / 2 - cx, viewport.h / 2 - cy);
    ...
    // Draw Snakes
    const drawShadows = false; // Disable for perf? Let's keep minimal glow
    
    // Sort so smaller are below bigger? Or just render
    (Object.values(gameState.snakes) as SnakeState[]).forEach(snake => {
        ...
        // Draw level bar
        const maxLevelVisual = 150; // just visual cap
        ...
        const eyeOffset = 6;
        const e1X = head.x + Math.cos(snake.dir + 0.5) * eyeOffset;
        const e1Y = head.y + Math.sin(snake.dir + 0.5) * eyeOffset;
        const e2X = head.x + Math.cos(snake.dir - 0.5) * eyeOffset;
        const e2Y = head.y + Math.sin(snake.dir - 0.5) * eyeOffset;
  ...
  return (
    <div className="flex flex-col w-full h-[100dvh] bg-neutral-950 font-sans select-none overflow-hidden touch-none relative">
      ...
      <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center z-10 bg-gradient-to-b from-black/80 to-transparent">
         <button onClick={onBackToLobby} className="w-10 h-10 bg-white/10 shrink-0 border border-white/20 rounded-full flex justify-center items-center backdrop-blur text-white hover:bg-white/20 active:scale-95 transition-all">
      ...
      {gameState?.phase === 'SETTINGS' && isHost && (
        <div className="flex-1 flex items-center justify-center relative p-6 animate-in fade-in zoom-in-95 duration-300">
           <div className="bg-neutral-900 border border-neutral-700/50 p-8 rounded-[2rem] w-full max-w-sm shadow-2xl text-center">
              ...
              <h2 className="text-3xl font-display font-bold text-white mb-2">Neon Snake</h2>
              <p className="text-neutral-400 font-medium mb-6">Slither to win! Choose your bot count to start.</p>
              
              <div className="space-y-6 mb-8 text-left">
                 <div>
                    <label className="text-neutral-400 text-sm font-bold uppercase tracking-wider mb-2 block">Bot Enemies</label>
                    <div className="flex items-center gap-3">
                       <input 
                         type="range" min="0" max="15" value={botCount} 
                         onChange={(e) => setBotCount(parseInt(e.target.value))}
                         className="flex-1 accent-fuchsia-500"
                       />
                       <span className="text-white font-mono font-bold text-xl w-8 text-center">{botCount}</span>
                    </div>
                 </div>
              </div>
      ...
      {gameState?.phase === 'SETTINGS' && !isHost && (
        <div className="flex-1 flex items-center justify-center relative p-6">
           <div className="animate-pulse flex flex-col items-center">
              ...
              <p className="text-neutral-400 font-medium text-lg">Waiting for Host to start...</p>
           </div>
        </div>
      )}
      ...
      {gameState?.phase === 'GAME_OVER' && (
         <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/60 backdrop-blur-sm p-6 animate-in fade-in duration-500">
             <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-3xl w-full max-w-sm shadow-2xl text-center transform hover:scale-105 transition-transform">
                ...
                <h2 className="text-4xl font-display font-black text-white tracking-tight mb-2 uppercase">{gameState.winner === 'Draw' ? 'Draw!' : `${gameState.winner} Wins!`}</h2>
                <p className="text-neutral-400 font-medium mb-8">Ready up in the lobby to play again.</p>
                {isHost ? (
                    <button onClick={onBackToLobby} className="w-full py-4 bg-white text-black font-bold rounded-2xl active:scale-95 transition-all">Return to Lobby</button>
                ) : (
                    <p className="text-neutral-500 font-bold uppercase tracking-widest text-sm">Waiting for host...</p>
                )}
             </div>
         </div>
      )}
        ...
        {gameState?.snakes[myId] && !gameState.snakes[myId].isDead && (
            <>
               <div className="absolute bottom-8 left-8 right-8 flex flex-col items-start pointer-events-none">
                   ...
                   <div className="mb-4">
                       <p className="text-white font-mono font-bold tracking-widest mb-1 text-shadow-sm">SCORE: {gameState.snakes[myId].score}</p>
                   </div>
                   ...
                   <div className="w-full max-w-[200px] h-3 bg-neutral-800/80 rounded-full border border-neutral-700/50 backdrop-blur overflow-hidden relative">
                      <div className="absolute left-0 top-0 bottom-0 bg-yellow-400 transition-all ease-linear" style={{ width: `${gameState.snakes[myId].boostAmount}%` }}></div>
                   </div>
               </div>
               ...
               <div className="absolute bottom-16 w-full flex justify-between px-8 pointer-events-none">
                  ...
                  <div className="pointer-events-auto">
                     ...
                     >
                        <span className="text-yellow-400 font-black tracking-widest uppercase text-sm drop-shadow-md">BOOST</span>
                     </button>
                  </div>
               </div>
            </>
